...
const uiText = {
  en: {
    ...
    connectedBinanceKey: "Connected Binance key",
    connectionManagement: "Connection management",
    connectionManagementHelp: "Change stored exchange credentials here. The import flow stays on the Binance page.",
    cancel: "Cancel",
    keyHint: "Key {key}",
  ...
  sv: {
    ...
    connectedBinanceKey: "Ansluten Binance-nyckel",
    connectionManagement: "Anslutningar",
    connectionManagementHelp: "Byt sparade bÃ¶rsuppgifter hÃ¤r. ImportflÃ¶det ligger kvar pÃ¥ Binance-sidan.",
    cancel: "Avbryt",
    keyHint: "Nyckel {key}",
        ...
        <form class="form-grid" data-form="profile">
          ...
          ${
            state.connection?.connected
              ? `<div class="settings-section">
                  <div class="settings-section-header">
                    <h3>${t("connectionManagement")}</h3>
                    <p class="muted">${t("connectionManagementHelp")}</p>
                  </div>
                  <div class="connection-summary compact-account-connection">
                    <div>
                      <strong>${escapeHtml(state.connection.label || t("mainBinance"))}</strong>
                      <span>${state.connection.apiKeyHint ? escapeHtml(tr("keyHint", { key: state.connection.apiKeyHint })) : t("connectedBinanceKey")}</span>
                    </div>
                    <button class="btn btn-secondary btn-fit" type="button" data-action="replace-binance-key">${t("replaceKey")}</button>
                  </div>
                </div>`
              : ""
          }
...
function handleAction(action) {
  ...
  if (action === "replace-binance-key") setState({ route: "binance", editingConnection: true, error: "", notice: "" });
